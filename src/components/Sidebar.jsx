...
export default function Sidebar() {
  ...
  return (
    <header
      id="top"
      className="pt-4 pb-10 sm:pt-8 sm:pb-12 md:sticky md:top-0 md:flex md:max-h-screen md:w-1/2 md:flex-col md:justify-between md:py-24"
    >
      <div>
        <img
          src="/raj-image.webp"
        ...
        />
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tightish text-text-light">
          {profile.name}
        </h1>
        <h2 className="mt-3 text-base sm:text-lg md:text-xl font-medium text-text">{profile.title}</h2>
        <p className="mt-4 max-w-xs text-sm sm:text-base leading-normal text-muted">{profile.tagline}</p>

        <p className="mt-6 max-w-md font-display text-xl leading-tight text-text-light sm:text-2xl">
          Building fast, resilient web products that turn complex systems into clear user experiences.
        </p>

        <div className="mt-7 grid max-w-md grid-cols-2 gap-x-5 gap-y-4 border-y border-border/70 py-5 sm:grid-cols-4 md:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label}>
              <p className="font-display text-xl font-semibold text-accent">{stat.value}</p>
              <p className="mt-1 text-[9px] uppercase leading-tight tracking-widest text-muted">{stat.label}</p>
            </div>
          ))}
        </div>

        <nav className="nav hidden md:block mt-12" aria-label="In-page navigation">
          <ul className="w-max">
            {NAV.map((n) => {
              ...
              return (
                <li key={n.href}>
                  <a
                    href={n.href}
                    className="group flex items-center py-2.5 active:text-text-light"
                  >
                    <span
